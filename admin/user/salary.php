...
<?php get_header(); ?>
<?php
if(!$monthly = get_form($_GET['id'])) { echo get_alert('maaş formu bulunamadı.'); return false; }
// eger odeme formu ise yonlendirme yap
if($monthly->type == 'payment') { header("Location: ".get_site_url('admin/payment/detail.php?id='.$monthly->id)); }
if($monthly->type != 'salary') { echo get_alert('maaş form "type" değeri "salary" olmalıdır.'); return false; }
if(!$user = get_user($monthly->user_id)) { echo get_alert('üye bilgileri bulunamadı'); return false; }

// gerekli kontrolleri yapalim ve tekrar cagiralim
set_staff_salary($monthly->user_id);
$monthly = get_form($_GET['id']);

$how_days = date("t", strtotime($monthly->date)); // ayda kac gun var
$price_day_wage = number_format($monthly->val_decimal / $how_days,4); // gunluk kac para
...
if(isset($_POST['update_monthly_salary'])) {
	echo $monthly_salary = get_set_decimal_db($_POST['monthly_salary']);

	if(db()->query("UPDATE ".dbname('forms')." SET val_decimal='".$monthly_salary."' WHERE id='".$monthly->id."' ")) {
		set_staff_salary($monthly->user_id);
	}
...
<div class="row">
	<div class="col-md-6">
		...
		<h4 class="content-title title-line">Çalışma durumu, mesai ekle/çıkar</h4>
		...
		<div class="h-20"></div>
		<h4 class="content-title title-line"><?php echo til_get_date($monthly->date, 'str: F Y'); ?> aylık maaş</h4>
	...
	<div class="col-md-6">
		...
		<div class="panel panel-default panel-table">
			<div class="panel-heading"><h3 class="panel-title"><?php echo til_get_date($monthly->date, 'str: F Y'); ?> Maaş Detayları</h3></div>
			<?php if($q_form_items->num_rows): ?>
				<table class="table table-hover table-condensed">
					...
					<tfooter>
						<tr>
							<td colspan="6" class="text-right"><?php echo get_set_money($monthly->total,true); ?></td>
						</tr>
					</tfooter>
				</table> <!-- /.table -->
			<?php else: ?>
