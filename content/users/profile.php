...
<?php
// Header Bilgisi
$breadcrumb[0] = array('name'=>'Form Yönetimi', 'url'=>get_site_url('content/accounts/'));
...
{
	unset($_POST['add_form']);
	$_POST['in_out'] = $in_out;
	$form_id = add_form($_POST);	
	if(!is_alert('add_form')) 
	{
		header("Location: ".get_site_url('content/forms/add.php?id='.$form_id));
	}
}
...
if(isset($_POST['form_submit']) and !is_uniquetime())
{
	...
	$location = false;
	if($form_id == 0){ $form_id = add_form($form); $location = true; }
	else { update_form($form_id, $form); }
	$form = get_form($form_id);
...
<div role="tabpanel" class="tab-pane fade" id="logs" aria-labelledby="logs-tab"> 
	...
	<?php if($lists = get_logs(array('table_id'=>'forms:'.@$form['id']))): ?>
